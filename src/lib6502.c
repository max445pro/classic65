...
/* B */ M_REL,  M_IND_Y,M_NONE, M_NONE, M_ZP_X, M_ZP_X, M_ZP_Y ,M_NONE, M_IMPL, M_ABS_Y,M_IMPL, M_NONE, M_ABS_X,M_ABS_X,M_ABS_Y,M_NONE,
...
uint16_t FETCH16_wrap(m6502 *cpu, uint8_t address) {
    uint8_t lo = FETCH(cpu, address);
    uint8_t hi = FETCH(cpu, (uint8_t)(address + 1)); // zero page wrap
    return (hi << 8) | lo;
}

uint16_t FETCH16_indirectbug(m6502 *cpu, uint16_t address) {
    uint8_t lo = FETCH(cpu, address);
    // Bug: high byte wraps in same page
    uint8_t hi = FETCH(cpu, (address & 0xFF00) | ((address + 1) & 0x00FF));
    return (hi << 8) | lo;
}

uint16_t FETCH16(m6502 *cpu, uint16_t address) {
    uint8_t lo = FETCH(cpu, address);
    uint8_t hi = FETCH(cpu, address + 1);
    return (hi << 8) | lo;
}
...
uint8_t pull(m6502 *cpu) {
    return FETCH(cpu, ++cpu->sp + 0x0100);
}
...
uint8_t addr_mode_size(uint8_t opcode) {
    switch (addr_modes[opcode]) {
        case M_A:
        case M_IMPL:
            return 1;
...
void bne(m6502 *cpu) {
    ...
    if (PC == 0x35db) { printf("adrf:%02x", FETCH(cpu, 0x0011)); exit(1); }
}
...
void jmp(m6502 *cpu) {
    PC = get_jump_location(cpu, OPCODE);
}
...
uint16_t breakpoints[16] = {0};
uint8_t top = 0;

void add_break_point(uint16_t address) {
    breakpoints[top++] = address;
}

/* ----- Execution ----- */

void check_break_points(m6502 *cpu) {
    for (int i = 0; i < 16; i++) {
        if (breakpoints[i] == PC) {printf("\nBreakpoint: %04x\n", PC), exit(1);}
    }
}
...
void execute_instr(m6502 *cpu, bool debug_info) {
    check_break_points(cpu);
    handle_interrupt(cpu);
    uint8_t opcode = FETCH(cpu, PC);
    if (debug_info) printf("pc: %04x, op: %02x, imm8: %02x, imm16: %04x, ", cpu->pc, FETCH(cpu, cpu->pc), FETCH(cpu, cpu->pc + 1), FETCH16(cpu, cpu->pc + 1));
    insn_table[opcode](cpu);
    if (debug_info) printf("A: %02x, X: %02x, Y: %02x, SR: %02x, SP: %02x\n", cpu->A, cpu->X, cpu->Y, cpu->sr, cpu->sp);
}
